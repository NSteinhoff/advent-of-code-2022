...
static int const space = 70000000;
static int const required = 30000000;
...
static int find(Entry const *entry, int tofree, int min) {
	if (entry->type == F) return min;
	if (entry->d.size < tofree) return min;
	min = entry->d.size < min ? entry->d.size : min;

	for (size_t i = 0; i < entry->d.len; i++)
		min = find(entry->d.entries[i], tofree, min);
...
static int solve(char *input) {
	...
	int tofree = required - (space - root->d.size);
	int freed = find(root, tofree, root->d.size);
	destroy(root);
